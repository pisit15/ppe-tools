...
import { ASSIGNMENT_KINDS, assignmentNodes, assignmentState, assignmentStateLabel, bangkokDate, validDate, type AssignmentNode } from '@/lib/team-assignments';
...
type Props = {
  ...
  onSavePositions: (points: Record<string, Point>, onSaved: (ids: string[]) => void) => Promise<void>;
...
export default function OrgWorkspace({ people: personnel, profiles: personProfiles, companies, onEdit, onSavePositions, onReload }: Props) {
  const [date, setDate] = useState(bangkokDate);
  const assignments = useMemo(() => assignmentNodes(personnel, personProfiles), [personnel, personProfiles]);
  const profiles = assignments.profiles;
  const people = useMemo(() => assignments.nodes.filter(n => assignmentState(n.assignment, date) === 'active'), [assignments, date]);
  ...
  const basePeople = useMemo(() => people.filter(p => {
    ...
    return (includeOutside || profile.in_scope !== false) && (!activeOnly || profile.status === 'working') && (!company || p.company_id === company) && (!team || profile.teams.includes(team)) && (!family || profile.family === family);
  ...
  const byId = useMemo(() => new Map(assignments.nodes.map(p => [p.id, p])), [assignments]);
  ...
  const headcount = new Set(basePeople.map(p => p.person_id)).size;
  const inactiveManagers = basePeople.flatMap(p => (['direct_manager','functional_manager'] as const).flatMap(type => {
    const manager = byId.get(p.assignment[type]);
    return manager && (assignmentState(manager.assignment, date) !== 'active' || !manager.is_active) ? [{ person: p, manager, type }] : [];
  }));
  const roleCaption = (p: AssignmentNode) => `${ASSIGNMENT_KINDS[p.assignment.kind]}${p.assignment.kind !== 'primary' ? ` · สังกัดหลัก ${p.primary_company_id.toUpperCase()}` : ''}`;
  ...
  function focusPerson(id: string) {
    ...
    const focusDate = person && assignmentState(person.assignment, date) !== 'active' ? person.assignment.start_date || person.assignment.end_date || date : date;
    setDate(focusDate);
    ...
    const candidates = assignments.nodes.filter(p => {
      const profile = profileFor(p);
      return assignmentState(p.assignment, focusDate) === 'active' && (includeOutside || person && profileFor(person).in_scope === false || profile.in_scope !== false) && (!activeOnly || person && profileFor(person).status !== 'working' || profile.status === 'working');
    });
  ...
  async function save() {
    ...
    let saved = 0;
    try {
      await onSavePositions(draft, ids => {
        setDraft(current => { const next = { ...current }; ids.forEach(id => delete next[id]); return next; });
        saved += ids.length;
      });
  ...
  function exportSvg() {
    ...
    const nodes = chartPeople.map(p => `<g transform="translate(${points[p.id].x},${points[p.id].y})"><rect width="${NODE_WIDTH}" height="${NODE_HEIGHT}" rx="12" fill="white" stroke="#cad7dd"/><text x="16" y="30" font-size="15" font-weight="600">${escapeSvg(short(p.full_name, 31))}</text><text x="16" y="57" font-size="13">${escapeSvg(short(p.position || 'ยังไม่ระบุตำแหน่ง', 34))}</text><text x="16" y="87" font-size="12" fill="#856019">${escapeSvg(roleCaption(p))}</text><text x="16" y="124" font-size="12" fill="#586976">${escapeSvg(p.company_id.toUpperCase() + ' · ' + profileFor(p).teams.join(' / '))}</text><title>${escapeSvg(p.full_name + ' · ' + p.position + ' · ' + roleCaption(p))}</title></g>`).join('');
    const note = `${date} · ${new Set(chartPeople.map(p => p.person_id)).size} คน · ${chartPeople.length} บทบาท · เส้นทึบ: สายบังคับบัญชา / เส้นประ: สายวิชาชีพ${chart.hidden.length ? ` · ${chart.hidden.length} สายรายงานไม่แสดงตามตัวกรอง/วันที่` : ''}`;
  ...
  return <section className={styles.workspace} aria-label="โครงสร้างทีม SHE และ ISO">
    ...
    <div className={styles.summary}><span><Users size={16}/><strong>{headcount}</strong> คน</span><span><strong>{basePeople.length}</strong> บทบาท</span><span><Building2 size={16}/><strong>{allCompanyCount}</strong> บริษัท</span><span><GitBranch size={16}/><strong>{allChart.edges.length}</strong> สายรายงาน</span><span className={styles.statusScope}>{activeOnly ? 'สถานะปัจจุบัน: ทำงานอยู่' : 'ทุกสถานะ'}{includeOutside ? ' · รวมคนนอกขอบเขต' : ''}</span></div>
    <div className={styles.toolbar}>
      ...
      <select aria-label="บริษัทของบทบาท" value={company} onChange={e => changeCompany(e.target.value)}><option value="">ทุกบริษัท</option>{companies.map(c => <option key={c.company_id} value={c.company_id}>{c.company_name || c.company_id.toUpperCase()}</option>)}</select>
      <label className={styles.dateFilter}>บทบาทที่มีผล ณ<input aria-label="วันที่ของการมอบหมายงาน" type="date" value={date} onChange={e => { if(validDate(e.target.value)){setDate(e.target.value);setSelectedId(null);setFocusId(null);resetCamera();} }}/></label>
    ...
    <div className={styles.viewBar}>
      ...
      <span className={styles.viewCaption}>{query ? `พบ ${found.length} บทบาท` : company ? companyName(company) : 'ทุกบริษัทในกลุ่ม EA'}{focusId && view === 'chart' && <button onClick={() => { setFocusId(null); resetCamera(); }}><X size={13}/>เลิกโฟกัส</button>}</span>
    </div>
    ...
    {inactiveManagers.length > 0 && <details className={styles.assignmentWarning}><summary>ต้องตรวจผู้รับช่วงต่อ {inactiveManagers.length} สายรายงาน · บทบาทผู้บังคับบัญชาไม่มีผลในวันที่เลือก หรือพ้นสถานะทำงาน</summary>{inactiveManagers.map(({person,manager,type}) => <button key={`${person.id}-${type}`} onClick={() => selectPerson(person.id)}>{person.full_name} ({person.company_id.toUpperCase()}) → {manager.full_name} · {manager.company_id.toUpperCase()} · {modeLabels[type]}</button>)}</details>}
    <div className={`${styles.workArea} ${selected ? styles.withInspector : ''}`}>
      <div ref={mainPanelRef} className={styles.mainPanel}>
        {view === 'overview' ? <div className={styles.overview}>
          ...
          {!found.length ? <div className={styles.empty}><Search size={30}/><h3>ไม่พบบุคลากรตามตัวกรอง</h3><p>ลองใช้ชื่อบางส่วน หรือเลือกดูทุกบริษัท</p><button onClick={clearFilters}>ล้างตัวกรอง</button></div> : <div className={styles.companyGrid}>{groups.map(id => {
            ...
            return <article className={styles.companyCard} key={id}><header><div className={styles.companyMonogram}>{id.toUpperCase().replace('EA-', '').slice(0,3)}</div><div><h3>{companyName(id)}</h3><span>{new Set(members.map(p => p.person_id)).size} คน · {members.length} บทบาท</span></div><button aria-label={`ดูผัง ${companyName(id)}`} title="ดูผังบริษัท" onClick={() => { changeCompany(id); setView('chart'); }}><ArrowRight size={18}/></button></header><div className={styles.companyPeople}>{visibleMembers.map(p => {
              const profile = profileFor(p);
              ...
              return <button key={p.id} className={`${styles.personRow} ${selectedId === p.id ? styles.selectedRow : ''}`} aria-pressed={selectedId === p.id} onClick={() => selectPerson(p.id)}><span className={`${styles.avatar} ${profile.teams.includes('ISO') ? styles.isoAvatar : ''}`}>{initials(p.full_name)}</span><span className={styles.personCopy}><strong>{p.full_name}</strong><span>{p.position || 'ยังไม่ระบุตำแหน่ง'}</span><span className={p.assignment.kind === 'acting' ? styles.actingMeta : styles.personMeta}>{roleCaption(p)}{reports > 0 && <span><GitBranch size={12}/>{reports}</span>}</span></span><ChevronRight size={15}/></button>;
            })}</div>{visibleMembers.length < members.length && <button className={styles.showMembers} onClick={() => changeCompany(id)}>ดูทั้งหมด {members.length} บทบาท<ArrowRight size={14}/></button>}</article>;
          })}</div>}
          <p className={styles.dataNote}>จัดกลุ่มตามบริษัทของบทบาท · คนเดียวแสดงได้หลายบริษัท แต่จำนวนคนนับรหัสบุคลากรไม่ซ้ำ · บริษัทที่รับผิดชอบเดิมจะยังไม่สร้างบทบาทรักษาการให้อัตโนมัติ</p>
          ...
          {chart.hidden.length > 0 && <div className={styles.contextNote}>{chart.hidden.length} สายรายงานไม่แสดงตามตัวกรองหรือช่วงเวลาของบทบาท <button onClick={clearFilters}>ดูทุกบริษัท</button></div>}
            ...
            <svg role="group" aria-label="ผังสายรายงาน ลากพื้นที่ว่างเพื่อเลื่อนผัง" className={styles.chartSvg} onPointerDown={e => { if (e.button !== 0) return; e.currentTarget.setPointerCapture(e.pointerId); pointer.current = { id: null, start: { x: e.clientX, y: e.clientY }, origin: { x: camera.x, y: camera.y } }; }} onPointerMove={e => {
              ...
              <g transform={`translate(${camera.x},${camera.y}) scale(${camera.scale})`}>
                ...
                {chartPeople.map(p => {
                  ...
                  return <g key={p.id} role="button" tabIndex={0} aria-label={`ดูข้อมูล ${p.full_name} · ${p.company_id.toUpperCase()} · ${ASSIGNMENT_KINDS[p.assignment.kind]}${editing ? ' ใช้ปุ่มลูกศรเพื่อย้าย' : ''}`} aria-pressed={selectedNode} className={styles.chartNode} transform={`translate(${point.x},${point.y})`} opacity={query && !matches(p) ? 0.28 : selectedId && !selectedNode && relatedIds.size > 0 && !relatedIds.has(p.id) ? 0.5 : 1} onClick={e => { e.stopPropagation(); if (!dragged.current) setSelectedId(p.id); dragged.current = false; }} onKeyDown={e => {
                    ...
                    <title>{p.full_name} · {p.position} · {companyName(p.company_id)} · {roleCaption(p)}</title>
                    ...
                    <rect x="16" y="69" width={p.assignment.kind === 'primary' ? 57 : 77} height="22" rx="5" fill={p.assignment.kind === 'acting' ? '#fff1d5' : '#edf3f5'}/><text x="23" y="84" fontSize="11" fill={p.assignment.kind === 'acting' ? '#835616' : '#526c7b'}>{ASSIGNMENT_KINDS[p.assignment.kind]}</text>
                    {p.assignment.kind !== 'primary' && <text x="103" y="84" fontSize="11" fill="#647681">สังกัดหลัก {p.primary_company_id.toUpperCase()}</text>}
                    <path d="M 16 104 H 264" stroke="#eef2f4"/><text x="16" y="127" fontSize="12" fill="#526c7b">{p.company_id.toUpperCase()}</text><text x="264" y="127" textAnchor="end" fontSize="12" fill="#68798a">{short(profile.teams.join(' / ') || 'ยังไม่จัดทีม', 27)}</text>
          ...
          <footer className={styles.canvasFooter}><span><i/>สายบังคับบัญชา</span><span><i className={styles.functionalLine}/>สายวิชาชีพ</span><span>{new Set(chartPeople.map(p => p.person_id)).size} คน · {chartPeople.length} บทบาท · {chart.edges.length} เส้น{dirtyCount > 0 ? ` · ${dirtyCount} ตำแหน่งยังไม่บันทึก` : ''}</span></footer>
        </div>}
      </div>
      {selected && selectedProfile && <aside className={styles.inspector} aria-label={`รายละเอียด ${selected.full_name}`} onKeyDown={e => { if (e.key === 'Escape') setSelectedId(null); }}>
        ...
        <div className={styles.inspectorActions}><button onClick={() => focusPerson(selected.id)}><Focus size={16}/>ดูสายรายงานที่เกี่ยวข้อง</button><button className={styles.primaryButton} onClick={() => { const person = personnel.find(p => p.id === selected.person_id); if(person)onEdit(person); }}><Pencil size={15}/>แก้ไขข้อมูล / การมอบหมายงาน</button></div>
        <dl className={styles.personFacts}><div><dt>บทบาทที่เลือก</dt><dd>{ASSIGNMENT_KINDS[selected.assignment.kind]} · {companyName(selected.company_id)}</dd></div><div><dt>สังกัดหลัก</dt><dd>{companyName(selected.primary_company_id)}</dd></div><div><dt>สถานะบุคลากรปัจจุบัน</dt><dd>{STATUS[selectedProfile.status]}</dd></div>{selected.assignment.kind !== 'primary' && <div><dt>ช่วงเวลามอบหมาย</dt><dd>{selected.assignment.start_date} — {selected.assignment.end_date || 'ไม่กำหนดสิ้นสุด'}<br/>{assignmentStateLabel[assignmentState(selected.assignment, date)]} ณ {date}</dd></div>}</dl>
        <div className={styles.relationships}><h3>ทุกบทบาทของบุคคลนี้</h3>{assignments.nodes.filter(p => p.person_id === selected.person_id).map(p => <button key={p.id} aria-pressed={selected.id === p.id} onClick={() => selectPerson(p.id)}><span>{p.company_id.toUpperCase()} · {ASSIGNMENT_KINDS[p.assignment.kind]}<small>{p.position || 'ยังไม่ระบุตำแหน่ง'} · {assignmentStateLabel[assignmentState(p.assignment, date)]}</small></span><ChevronRight size={15}/></button>)}</div>
        <div className={styles.relationships}><h3>รายงานต่อ</h3>{(['direct_manager','functional_manager'] as const).map(type => {
          ...
          return <div key={type}><span className={styles.relationshipLabel}><i className={type === 'functional_manager' ? styles.functionalLine : ''}/>{modeLabels[type]}</span>{manager ? <button onClick={() => selectPerson(manager.id)}><span>{manager.full_name}<small>{manager.company_id.toUpperCase()} · {ASSIGNMENT_KINDS[manager.assignment.kind]} · {manager.position}</small>{assignmentState(manager.assignment, date) !== 'active' && <small className={styles.actingMeta}>{assignmentStateLabel[assignmentState(manager.assignment, date)]} — ตรวจผู้รับช่วงต่อ</small>}</span><ChevronRight size={15}/></button> : <p>{selectedProfile[type] ? 'บทบาทอยู่นอกชุดข้อมูลนี้' : 'ยังไม่ระบุผู้บังคับบัญชา'}</p>}</div>;
        })}</div>
